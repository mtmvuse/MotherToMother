...
const ReportsPage: React.FC = () => {
	...
	useEffect(() => {
		...
		if (rootRef.current) {
			rootRef.current.render(<FooterSummary totalQuantity={totalQuantity} totalValue={totalValue} />);
		}
	...
	const handleFilterModelChange = (model: GridFilterModel) => {
		...
		if (currFilterArray && newFilterArray) {
			...
			let addFilterArray = currFilterArray.filter((item) => item?.field !== searchField);
	...
	const reportQueryResponse = useQuery({
		...
		queryFn: () =>
			currentUser
				?.getIdToken()
				.then((token) => getReports(token, page, pageSize, filterModel, sortModel))
