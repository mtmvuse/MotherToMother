...
import type { AddCashDonationType, CashDonationRow, EditCashArgs, cdDashboardResponse } from '~/types/cashDonationTypes';
...
const CashDonationsPage: React.FC = () => {
	...
	const [openDeleteCashDonation, setOpenDeleteCashDonation] = React.useState(false);
	const [deleteRow, setDeleteRow] = React.useState<CashDonationRow | undefined>();
	...
	const editMutation = useMutation({
		mutationFn: (data: EditCashArgs) => updateCashDonation(data.id, data.cashData, 'token'),
		onSuccess: (result: Response) => {
			queryClient.invalidateQueries({ queryKey: ['cashDonation'] });
			if (result.status === 400 || result.status === 500) {
	...
	const findOrganizationId = (organizationName: string) => {
		return organizationsQueryResponse.data?.find((organization) => organization.name === organizationName)?.id;
	};
	...
	if (cdQueryResponse.error) return <ErrorMessage error={cdQueryResponse.error.message} setError={setError} />;
	...
	return (
		<Box>
			...
			<div style={{ display: 'flex ' }}>
				...
				<Button className='table-add-button' onClick={handleOpenAddCashDonation} endIcon={<AddIcon />}>
					Add
				</Button>
			</div>
