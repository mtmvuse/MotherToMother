...
const FiltersOptions: React.FC<FiltersOptionsProps> = ({ filter, filterModel, setFilterModel }) => {
	const handleFilterDelete = () => {
		let currFilterArray = filterModel?.items;
		let newFilterArray: GridFilterItem[] = [];

		if (currFilterArray) {
			newFilterArray = currFilterArray.filter((item) => item?.field !== filter.field);
		}
	...
	return (
		<Box className='filterOption-container'>
			<Typography className='filterOption-text'>
				{filter.field}
				<IconButton onClick={handleFilterDelete} aria-label='clear-date-filter' style={{ color: 'lightgray' }}>
					<CancelIcon />
				</IconButton>
			</Typography>
		</Box>
